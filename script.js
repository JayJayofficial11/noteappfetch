...
function createNoteElement(noteText, id) {
    ...
    editButton.addEventListener('click', function() {
        ...
        else updateNoteAPI(note.dataset.id, input.value.trim());
    });
...
// Function to update note on API
function updateNoteAPI(noteId, noteText) {
    const updatedNote = { id: noteId, text: noteText };
    // Here you can make an API call to update the note with the given noteId
    // For example: fetch("https://api.example.com/notes/" + noteId, { method: 'PUT', body: JSON.stringify(updatedNote) })
}

// Function to delete note from API
function deleteNoteAPI(noteId) {
    // Here you can make an API call to delete the note with the given noteId
    // For example: fetch("https://api.example.com/notes/" + noteId, { method: 'DELETE' })
}
